...
int canConfig3();
int pollTimer_config3(struct pollfd * ufds);
int pollTimer_set3(const long long tValueNs, const long long tIntervalNs, struct pollfd * ufds);
long long execTime_count3( struct timespec * timeStartPtr, struct timespec * timeStopPtr);

//-----------------------------------
// main
int Del2main(int argc, char *argv[])
{
	int i;
	int canSocket;
	int canConfigFlag = 1;
//*
	if(!(canSocket = canConfig3()))
	{
...
//*/
	...
	//-------------------------
	// timer
	pollTimer_config3(ufds);
	pollTimer_set3(5000000000, 5000000000, ufds);
...
//*
		else if (ufds[2].revents & POLLIN)
		{
			...
			execTime = execTime_count3( &timeStampOld, &timeStampNew);
			...
			printf("\n");
			printf("time from last can incoming frame: %lld ns\n", execTime);
		}
//*/
	}
...
long long execTime_count3( struct timespec * timeStartPtr, struct timespec * timeStopPtr)
{
	...
	cpt_s = ((long long)(timeStopPtr->tv_sec - timeStartPtr->tv_sec)) * NANO_IN_SEC;
