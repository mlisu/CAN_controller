...
long long execTime_count( struct timespec * timeStartPtr, struct timespec * timeStopPtr);
...
//*/	
	...
	//-------------------------
	// timer
	pollTimer_config(ufds);
	pollTimer_set(5000000000, 5000000000, ufds);
...
//*		
		...
		{	
			...
			execTime = execTime_count( &timeStampOld, &timeStampNew);			
